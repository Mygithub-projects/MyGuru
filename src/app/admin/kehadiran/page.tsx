...
const JENIS = ["Sukan", "Kelab", "Uniform"];
...
export default async function AdminKehadiranPage({
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/admin" className={back}>← Kembali</Link>
        <h1 className="mt-1 text-xl font-bold text-slate-800">Kehadiran — Setiap Perjumpaan</h1>
        <p className="text-sm text-slate-500">Klik mana-mana baris untuk melihat senarai nama hadir/tidak hadir.</p>
      </div>

      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <Kad label="Jumlah Perjumpaan" nilai={`${rows.length}`} />
        <Kad label="Purata Kehadiran" nilai={`${purata}%`} />
        <Kad label="Belum Disahkan" nilai={`${rows.filter((r) => !r.disahkan).length}`} />
      </div>
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
        <form method="get" className="flex flex-wrap items-end gap-3">
          ...
          <label className="text-xs font-medium text-slate-600">Jenis
            <select name="jenis" defaultValue={fJenis} className={`${inp} mt-1 block`}>
              <option value="">Semua</option>
              {JENIS.map((j) => <option key={j} value={j}>{j}</option>)}
            </select>
          </label>
          <button type="submit" className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white hover:bg-brand-hover">Tapis</button>
          {(fUnit || fJenis) && <Link href="/admin/kehadiran" className="px-2 py-2 text-sm text-slate-500 hover:underline">Reset</Link>}
        </form>
      ...
      <section className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        <h2 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-600">Senarai Perjumpaan</h2>
        {rows.length === 0 ? (
          <p className="text-sm text-slate-400">Tiada sesi kehadiran untuk tapisan ini.</p>
        ) : (
          <KehadiranTable rows={rows} />
        )}
      </section>
    </div>
  );
